...
public class CommandLineRunner extends AbstractCommandLineRunner<Compiler, CompilerOptions>
{
	...
	{
		private static final WarningGuardSpec warningGuardSpec = new WarningGuardSpec();
		...
		@Option(name = "--define", aliases =
		{ "--D", "-D" }, usage = "Override the value of a variable annotated @define. "
		...
		@Option(name = "--use_types_for_optimization", usage = "Experimental: perform additional optimizations "
				+ "based on available information.  Inaccurate type annotations " + "may result in incorrect results.")
		...
		@Option(name = "--manage_closure_dependencies", handler = BooleanOptionHandler.class, usage = "Automatically sort dependencies so that a file that "
				...
				+ "those symbols are never required, then that input will not " + "be included in the compilation.")
		...
		@Option(name = "--only_closure_dependencies", handler = BooleanOptionHandler.class, usage = "Only include files in the transitive dependency of the "
				...
				+ "not provide dependencies will be removed. This supersedes" + "manage_closure_dependencies")
		...
		/**
		 * Users may specify JS inputs via the legacy {@code --js} option, as
		 * well as via additional arguments to the Closure Compiler. For
		 * example, it is convenient to leverage the additional arguments
		 * feature when using the Closure Compiler in combination with {@code
		 * find} and {@code xargs}:
		 * 
		 * <pre>
		 ...
		 * <p>
		 * Note that it is important to use the {@code
		 * --manage_closure_dependencies} option in this case because the order
		 * produced by {@code find} is unlikely to be sorted correctly with
	...
	private void initConfigFromFlags(String[] args, PrintStream err)
	{
		...
		if (flags.process_common_js_modules)
		{
			...
			flags.closure_entry_point = Lists.newArrayList(ProcessCommonJSModules
					.toModuleName(flags.common_js_entry_module));
		}
		...
		else
		{
			...
			getCommandLineConfig().setPrintTree(flags.print_tree).setPrintAst(flags.print_ast).setPrintPassGraph(
					flags.print_pass_graph).setJscompDevMode(flags.jscomp_dev_mode)
					...
					.setJsOutputFile(flags.js_output_file).setModule(flags.module).setVariableMapInputFile(
							flags.variable_map_input_file).setPropertyMapInputFile(flags.property_map_input_file)
					.setVariableMapOutputFile(flags.variable_map_output_file).setCreateNameMapFiles(
							flags.create_name_map_files).setPropertyMapOutputFile(flags.property_map_output_file)
					.setCodingConvention(conv).setSummaryDetailLevel(flags.summary_detail_level).setOutputWrapper(
							flags.output_wrapper).setModuleWrapper(flags.module_wrapper).setModuleOutputPathPrefix(
							flags.module_output_path_prefix).setCreateSourceMap(flags.create_source_map)
					.setSourceMapFormat(flags.source_map_format).setWarningGuardSpec(Flags.warningGuardSpec).setDefine(
							flags.define).setCharset(flags.charset).setManageClosureDependencies(
							flags.manage_closure_dependencies).setOnlyClosureDependencies(
							flags.only_closure_dependencies).setClosureEntryPoints(flags.closure_entry_point)
					.setOutputManifest(ImmutableList.of(flags.output_manifest)).setOutputModuleDependencies(
							flags.output_module_dependencies).setAcceptConstKeyword(flags.accept_const_keyword)
					.setLanguageIn(flags.language_in).setProcessCommonJSModules(flags.process_common_js_modules)
					.setCommonJSModulePathPrefix(flags.common_js_path_prefix).setTransformAMDToCJSModules(
							flags.transform_amd_modules).setWarningsWhitelistFile(flags.warnings_whitelist_file);
		}
	}
	...
	private static final List<String> DEFAULT_EXTERNS_NAMES = ImmutableList.of(
			...
			// Event APIs
			"w3c_event.js", "w3c_event3.js", "gecko_event.js",
			"ie_event.js",
			"webkit_event.js",

			// DOM apis
			"w3c_dom1.js", "w3c_dom2.js", "w3c_dom3.js", "gecko_dom.js",
			"ie_dom.js",
			"webkit_dom.js",

			// CSS apis
			"w3c_css.js", "gecko_css.js",
			"ie_css.js",
			"webkit_css.js",
	...
	public static List<SourceFile> getDefaultExterns() throws IOException
	{
		...
		if (input == null)
		{
			...
			String name = path + File.separator + "externs.zip";
	...
	public static void main(String[] args)
	{
		...
		input[0] = "--js=D:\\script\\FusionCharts.js";
		input[1] = "--charset=gbk";
		input[2] = "--js_output_file=D:\\scriptout\\FusionCharts.js";
